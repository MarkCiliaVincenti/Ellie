...
";
    ...
    private EllieExpression[] globalReactions;
    private ConcurrentDictionary<ulong, EllieExpression[]> newGuildReactions;
    ...
    private ValueTask OnGexprAdded(EllieExpression c)
    {
        lock (_gexprWriteLock)
        {
            var newGlobalReactions = new EllieExpression[globalReactions.Length + 1];
    ...
    {
        if (maybeGuildId is { } guildId)
            return newGuildReactions.TryGetValue(guildId, out var exprs) ? exprs : Array.Empty<EllieExpression>();
