...
namespace Ellie.Modules.Games.Services;
                ...
                if (maybeLimit.TryPickT1(out var ql, out var counters))
                {
                    if (ql.Quota == 0)
                    {
                        await channel.SendErrorAsync(_eb,
                            null!,
                            text:
                            "In order to use the cleverbot feature, the owner of this server should be [Patron Tier X](https://patreon.com/join/Ellie) on patreon.",
                            footer:
