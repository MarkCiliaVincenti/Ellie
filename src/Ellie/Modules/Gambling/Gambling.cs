...
using Ellie.Modules.Utility.Patronage;
...
public partial class Gambling : GamblingModule<GamblingService>
{
    ...
    private EllieButtonInteraction CreateCashInteraction()
...
";
    ...
    public async partial Task Rps(RpsPick pick, ShmartNumber amount = default)
    {
        ...
        var ElliePick = (RpsPick)new EllieRandom().Next(0, 3);
        ...
        else if ((pick == RpsPick.Paper && ElliePick == RpsPick.Rock)
                 || (pick == RpsPick.Rock && ElliePick == RpsPick.Scissors)
                 || (pick == RpsPick.Scissors && ElliePick == RpsPick.Paper))
        {
            ...
            msg = GetText(strs.rps_win(ctx.User.Mention, GetRpsPick(pick), GetRpsPick(ElliePick)));
        }
        else
        {
            embed.WithErrorColor();
            msg = GetText(strs.rps_win(ctx.Client.CurrentUser.Mention, GetRpsPick(ElliePick), GetRpsPick(pick)));
        }
