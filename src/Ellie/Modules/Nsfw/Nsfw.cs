...
#if !GLOBAL_ELLIE
[NoPublicBot]
public partial class NSFW : EllieModule<ISearchImagesService>
{
    ...
    private async Task InternalBoobs()
    {
        try
        {
            ...
            {
                obj = JArray.Parse(
                    await http.GetStringAsync($"http://api.oboobs.ru/boobs/{new EllieRandom().Next(0, 10330)}"))[0];
            }
    ...
    private async Task InternalButts(IMessageChannel channel)
    {
        try
        {
            ...
            {
                obj = JArray.Parse(
                    await http.GetStringAsync($"http://api.obutts.ru/butts/{new EllieRandom().Next(0, 4335)}"))[0];
            }
    ...
    public async Task Boobs()
    {
        try
        {
            ...
            {
                obj = JArray.Parse(
                    await http.GetStringAsync($"http://api.oboobs.ru/boobs/{new EllieRandom().Next(0, 12000)}"))[0];
            }
    ...
    public async Task Butts()
    {
        try
        {
            ...
            {
                obj = JArray.Parse(
                    await http.GetStringAsync($"http://api.obutts.ru/butts/{new EllieRandom().Next(0, 6100)}"))[0];
            }
